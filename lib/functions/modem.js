...
module.exports = function(SerialPort) {
  ...
  modem.open = function(device, options, callback) {
    ...
    if (options.deliveryReport !== undefined) {
        modem.deliveryReport = options.deliveryReport
        delete options.deliveryReport
    }
  ...
  modem.deliverSMSById = function (id) {
    modem.executeCommand('AT+CPMS="SR"').then(() => {
      const deliver = modem.executeCommand(`AT+CMGR=${id}`, () => {}, true)
      let resultData;
      deliver.logic = (newpart) => {
        let regx = /[0-9A-Fa-f]{15}/g
        if (regx.test(newpart)) {
          let indexDelivery = parseInt(id, 10)
          let reference
          try {
            let newMessage = pdu.parse(newpart)
            reference = newMessage.getReference()
            resultData = {
              sender: newMessage.getAddress().getPhone(),
              timeDelivered: new Date(newMessage.getDischarge().getTime() * 1000),
              index: indexDelivery,
              reference
            }
          } catch (error) {
          ...
          modem.emit('onMessageDelivered', resultData)

          modem.checkSimMemory(true)
        }
        regx.lastIndex = 0 // be sure to reset the index after using .test()
        if ((newpart === '>' || newpart === 'OK') && resultData) {
          return {
    ...
    parts.forEach(part => {
      ...
      newparts.forEach(newpart => {
        ...
        if (modem.incomingSMSIndication && newpart.startsWith('+CMTI:')) { // New Message Indicator with SIM Card ID, After Receiving Read The Message From the SIM Card
          ...
          modem.checkSimMemory()
        } else if (newpart.startsWith('+CDSI:')) {
          const splitted_newpart = newpart.split(',')
          modem.deliverSMSById(splitted_newpart[1])
        } else if (newpart.startsWith('+CMGS:')) {
          if (modem.queue[0] && modem.queue[0].status === 'Sending SMS') {
            modem.queue[0].reference = +newpart.slice(7)
          }
        }
          ...
          if ((modem.queue[0].status === 'sendSMS')) { // If SMS is currently Sending Emit currently sending SMS
            modem.emit('onSendingMessage', {
              ...
              data: {
                ...
                reference: modem.queue[0].reference,
  ...
  self.processMessage = (message, messageIndex, messageStatus) => {
    ...
    let newMessage = {
      ...
      message: messageData ? messageData.getText() : null,
      index: messageIndex,
    ...
    if (messageData && messageData.getParts()[0].getHeader()) {
      newMessage.udh = {
        referenceNumber: messageData.getParts()[0].getHeader().getPointer(),
        parts: messageData.getParts()[0].getHeader().getSegments(),
        part: messageData.getParts()[0].getHeader().getCurrent()
      }
    }
